#!/usr/bin/env python3
"""
poc1.py — POC-1: Benchmark VLMs trên ViInfographicVQA
=======================================================

Entry point duy nhất. Ghép các module lại theo thứ tự pipeline:
    Step 1: Load data          (modules/data/loader.py)
    Step 2: Batch inference    (modules/inference/runner.py)
    Step 3: Evaluate           (modules/evaluation/analyzer.py)
    Step 4: Report             (modules/reporting/table.py + charts.py)

Cách chạy:
    # Dev mode (20 mẫu, 1 model — test pipeline nhanh)
    python poc1.py --dev --models vintern-1b

    # Chạy 1 model cụ thể (full dataset)
    python poc1.py --models qwen25vl-7b

    # Chạy tất cả models enabled trong config
    python poc1.py

    # Đã có predictions rồi, chỉ evaluate + report lại
    python poc1.py --skip-inference

    # Dùng config khác
    python poc1.py --config configs/my_config.yaml
"""
...
# ── sys.path: đảm bảo import modules/ từ thư mục poc1/ ──────────────
_ROOT = Path(__file__).resolve().parent
sys.path.insert(0, str(_ROOT))

from modules.data       import loader, sampler
from modules.inference  import runner
from modules.evaluation import analyzer
from modules.reporting  import table, charts
...
def _setup_logging(log_dir: str) -> None:
    ...
    log_file = Path(log_dir) / f"poc1_run_{ts}.log"
...
def _load_config(path: str) -> dict:
    ...
    if not cfg_path.exists():
        sys.exit(f"[poc1] Config file not found: {cfg_path}")
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="POC-1: Benchmark VLMs trên ViInfographicVQA",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    p.add_argument(
        "--config",
        default=str(_ROOT / "configs" / "poc1_config.yaml"),
        help="Đường dẫn file config YAML (default: configs/poc1_config.yaml)",
    )
    p.add_argument(
        "--models",
        nargs="+",
...
def main() -> int:
    ...
    log.info("POC-1 START — VLM Benchmark on ViInfographicVQA")
